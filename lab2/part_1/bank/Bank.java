...
abstract class AbstractBank implements Bank, TestableBank {
    ...
    protected static final String BRIGHT_YELLOW = "[93m";
    protected static final String BRIGHT_CYAN   = "[96m";
    ...
    public void test() {
        ...
        System.out.println(
                "  " + MAGENTA + getClass().getSimpleName() + RESET
                        + DIM + " → transactions: " + RESET
                        + BRIGHT_YELLOW + ntransacts + RESET
                        + DIM + ", sum: " + RESET
                        + BRIGHT_CYAN + sum + RESET);
    }
}
...
class AtomicBank implements Bank, TestableBank {
    ...
    public void test() {
        ...
        try {
            ...
            boolean correct = sum == expectedTotal;
            System.out.println(
                    "  " + MAGENTA + "AtomicBank" + RESET
                            + DIM + " → transactions: " + RESET
                            + BRIGHT_YELLOW + ntransacts.get() + RESET
                            + DIM + ", sum: " + RESET
                            + (correct ? BRIGHT_GREEN : BRIGHT_RED) + sum + RESET
                            + DIM + ", expected: " + RESET
                            + BRIGHT_CYAN + expectedTotal + RESET);
        } finally {
            lock.unlock();
        }
    }
