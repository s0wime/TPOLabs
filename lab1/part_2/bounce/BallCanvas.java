...
public class BallCanvas extends JPanel {
    ...
    public void setPocketedLabel(JLabel label) { this.pocketedLabel = label; }
    public int getPocketedCount() { return pocketedCount; }

    private List<Point> getPocketCenters() {
        int w = getWidth(), h = getHeight(), in = Config.POCKET_INSET;
        return List.of(
            new Point(in, in), new Point(w - in, in),
            new Point(in, h - in), new Point(w - in, h - in),
            new Point(w / 2, in), new Point(w / 2, h - in));
    }
    ...
    public boolean isBallInPocket(Ball ball) {
        ...
        for (Point p : getPocketCenters())
            if (Math.hypot(cx - p.x, cy - p.y) <= Config.POCKET_RADIUS) return true;
        return false;
    }
        ...
        synchronized (balls) {
            if (balls.remove(ball)) {
                ...
                if (pocketedLabel != null)
                    SwingUtilities.invokeLater(() -> pocketedLabel.setText("В лузі: " + count));
    ...
    private void drawPockets(Graphics2D g2) {
        ...
        for (Point p : getPocketCenters()) {
            ...
            g2.setColor(new Color(120, 80, 255, 160));
            g2.drawOval(p.x - r, p.y - r, 2 * r, 2 * r);
            // Inner glow
            RadialGradientPaint pocketGlow = new RadialGradientPaint(p.x, p.y, r * 1.4f,
                new float[]{0f, 1f},
                new Color[]{new Color(100, 60, 220, 40), new Color(0, 0, 0, 0)});
            g2.setPaint(pocketGlow);
            g2.fillOval(p.x - r - 4, p.y - r - 4, 2 * r + 8, 2 * r + 8);
            g2.setStroke(new BasicStroke(1f));
        }
    }
    ...
    private void updateSystemStats() { processCpuLoad = osBean.getProcessCpuLoad() * 100.0; }
    ...
    private void drawOverlay(Graphics2D g2) {
        ...
        drawStat(g2, tx, ty, "On table", String.valueOf(balls.size()), new Color(80, 200, 255)); ty += 17;
        drawStat(g2, tx, ty, "Pocketed", String.valueOf(pocketedCount), new Color(200, 160, 255)); ty += 17;
    ...
    private void drawFilledGraph(Graphics2D g2, List<? extends Number> data,
        ...
        area.moveTo(x, y + height);
        area.lineTo(x, y + height - (data.get(0).floatValue() * height / maxValue));
        for (int i = 1; i < n; i++) {
            float px = x + i * step;
        ...
        line.moveTo(x, y + height - (data.get(0).floatValue() * height / maxValue));
        for (int i = 1; i < n; i++) {
            float px = x + i * step;
            float py = y + height - (data.get(i).floatValue() * height / maxValue);
            line.lineTo(px, py);
